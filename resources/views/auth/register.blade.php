    ...
    <form method="POST" novalidate action="{{ route('register') }}">
        ...
        <div class="mt-4" x-data="{ show: false }">
            ...
            <div class="relative mt-1">
                <!-- เปลี่ยน type เป็น x-bind:type เพื่อให้ Alpine.js จัดการแทน และเพิ่มคลาส pr-10 ให้ข้อความไม่ทับไอคอน -->
                <x-text-input id="password" class="block w-full pr-10" x-bind:type="show ? 'text' : 'password'"
                    name="password" required autocomplete="new-password" />

                <button type="button" @click="show = !show"
                    class="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-500 hover:text-gray-700 focus:outline-none">
                    <!-- ไอคอนสำหรับแสดงรหัส (ตาถูกขีดฆ่า) -->
