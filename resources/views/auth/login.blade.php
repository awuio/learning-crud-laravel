<x-guest-layout>
    ...
    <!-- 1. เพิ่ม x-data="{ isSubmitting: false }" และ @submit="isSubmitting = true" ที่ฟอร์ม -->
    <form method="POST" action="{{ route('login') }}" novalidate x-data="{ isSubmitting: false }" @submit="isSubmitting = true">
        ...
        <div class="flex items-center justify-end mt-4">
            ...
            <!-- 2. อัปเดตปุ่ม Login ให้ตอบสนองต่อตัวแปร isSubmitting -->
            <x-primary-button class="ms-3 flex items-center justify-center transition-all"
                x-bind:disabled="isSubmitting" x-bind:class="{ 'opacity-75 cursor-wait': isSubmitting }">

                <!-- SVG Loading (จะหมุนเมื่อ isSubmitting เป็น true) -->
                ...
                <span x-show="isSubmitting" style="display: none;">{{ __('กำลังเข้าสู่ระบบ...') }}</span>
            </x-primary-button>
        </div>
    </form>
</x-guest-layout>
